# bdb-2026 <!-- omit in toc -->

This repository contains files used in the course "Biomedical Data Bases" (BDB) Module 2
at the University of Bologna, Academic Year 2025-2026, taught by prof. Davide Salomoni.

For details, see the course slides.

For more information on the course, see [here](https://www.unibo.it/en/study/course-units-transferable-skills-moocs/course-unit-catalogue/course-unit/2025/524686).
...
- [4. Possible issues with Docker and how to fix them:](#4-possible-issues-with-docker-and-how-to-fix-them)
  - [4.1. `docker run` returns `docker: Error response from daemon: Conflict. The container name "/my_jupyter" is already in use ...`](#41-docker-runreturns-docker-error-response-from-daemon-conflict-the-container-name-my_jupyter-is-already-in-use-)
  - [4.2. On Windows, you get the message `"Error response ... : file exists."`](#42-on-windows-you-get-the-message-error-response---file-exists)
  - [4.3. On Windows, after you install Docker Desktop you cannot run VirtualBox VMs anymore](#43-on-windows-after-you-install-docker-desktop-you-cannot-run-virtualbox-vms-anymore)
  - [4.4. On Windows, you get the message `"docker_engine: Access is denied"`](#44-on-windows-you-get-the-message-docker_engine-access-is-denied)
  - [4.5. On Windows, you get the message `"Docker failed to initialize"` when running Docker Desktop](#45-on-windows-you-get-the-message-docker-failed-to-initialize-when-running-docker-desktop)
  - [4.6. On Windows, a message says that you need to enable virtualization](#46-on-windows-a-message-says-that-you-need-to-enable-virtualization)
  - [4.7. Other errors](#47-other-errors)
...
## 4. Possible issues with Docker and how to fix them:

### 4.1. `docker run` returns `docker: Error response from daemon: Conflict. The container name "/my_jupyter" is already in use ...`

- Type `docker stop my_jupyter`
- Type `docker rm my_jupyter`
- Type the `docker run` command again.

### 4.2. On Windows, you get the message `"Error response ... : file exists."`
...
### 4.3. On Windows, after you install Docker Desktop you cannot run VirtualBox VMs anymore
...
### 4.4. On Windows, you get the message `"docker_engine: Access is denied"`
...
### 4.5. On Windows, you get the message `"Docker failed to initialize"` when running Docker Desktop
...
### 4.6. On Windows, a message says that you need to enable virtualization
